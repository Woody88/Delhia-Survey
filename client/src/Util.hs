...
headSection cssPath cssFiles =
  ...
  in do
    ...
    traverse_ (\f -> stylesheet . Text.pack $ cssPath </> "css" </> f) cssFiles

serveCss :: FilePath -> (forall x. Widget x ()) -> IO (W.Application, JSM ())
serveCss assetsPath w = do
  cssFiles <- listDirectory $ "." </> assetsPath </> "css"
  let
    frontendApp =
      mainWidgetWithHead (headSection assetsPath cssFiles) w
    backendApp  =
      staticPolicy $ hasPrefix assetsPath
